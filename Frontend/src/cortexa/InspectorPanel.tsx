      ...
      <div className="border-b border-border px-4 py-3">
        <div className="flex items-center gap-2.5">
          <div className={cn(
            ...
            meta.accent === "green" && "border-accent/40 bg-accent/10 text-accent",
            meta.accent === "red" && "border-destructive/40 bg-destructive/10 text-destructive"
          )}>
        ...
        <TabsContent value="config" className="flex-1 overflow-hidden m-0">
          <ScrollArea className="h-full">
            <div className="space-y-4 px-4 pb-6">
              ...
              {/* ── Web Search ──────────────────────────────────────────── */}
              {node.data.kind === "tool.web-search" && (
                <div className="space-y-2">
                  <div>
                    <Label className="text-xs text-muted-foreground">Provider</Label>
                    <Select
                      value={node.data.config.provider ?? "auto"}
                      onValueChange={(v) => onConfigChange(node.id, { provider: v })}
                    >
                      <SelectTrigger className="mt-1 bg-surface-elevated"><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="auto">auto (Tavily → DuckDuckGo)</SelectItem>
                        <SelectItem value="tavily">tavily (needs key)</SelectItem>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      value={node.data.config.maxResults ?? 5}
                      onChange={(e) => onConfigChange(node.id, { maxResults: Number(e.target.value) })}
                      className="mt-1 bg-surface-elevated"
                    />
                  </div>
                </div>
              ...
              {/* ── Document Ingest ─────────────────────────────────────── */}
              {node.data.kind === "memory.ingest" && (
                <div className="space-y-2">
                  <div>
                    <Label className="text-xs text-muted-foreground">Mode</Label>
                    <Select
                      value={node.data.config.mode ?? "auto"}
                      onValueChange={(v) => onConfigChange(node.id, { mode: v })}
                    >
                      <SelectTrigger className="mt-1 bg-surface-elevated"><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <Input
                        ...
                        value={node.data.config.chunkSize ?? 800}
                        onChange={(e) => onConfigChange(node.id, { chunkSize: Number(e.target.value) })}
                        className="mt-1 bg-surface-elevated"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        value={node.data.config.chunkOverlap ?? 100}
                        onChange={(e) => onConfigChange(node.id, { chunkOverlap: Number(e.target.value) })}
                        className="mt-1 bg-surface-elevated"
                      />
                    </div>
                  </div>
              ...
              {/* ── PII Guard ───────────────────────────────────────────── */}
              {node.data.kind === "safety.pii-guard" && (
                <div>
                  <Label className="text-xs text-muted-foreground">Action</Label>
                  <Select
                    value={node.data.config.action ?? "mask"}
                    onValueChange={(v) => onConfigChange(node.id, { action: v })}
                  >
                    <SelectTrigger className="mt-1 bg-surface-elevated"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="mask">mask (redact in place)</SelectItem>
                      <SelectItem value="block">block (fail on PII)</SelectItem>
                      <SelectItem value="report">report (flag only)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              )}
              ...
              {/* ── Eval Graders ────────────────────────────────────────── */}
              {node.data.kind === "logic.grader" && (
                <div className="space-y-2">
                  <div>
                    <Label className="text-xs text-muted-foreground">
                      Pass Threshold ({node.data.config.threshold ?? 7}/10)
                    </Label>
                    <Input
                      type="number" min={0} max={10} step={0.5}
                      value={node.data.config.threshold ?? 7}
                      onChange={(e) => onConfigChange(node.id, { threshold: Number(e.target.value) })}
                      className="mt-1 bg-surface-elevated"
                    />
                  </div>
                  <div>
                    <Label className="text-xs text-muted-foreground">House Rubric (optional)</Label>
                    <Textarea
                      rows={3} value={node.data.config.rubric ?? ""}
                      placeholder="e.g. Tone must be confident, claims must cite sources…"
                      onChange={(e) => onConfigChange(node.id, { rubric: e.target.value })}
