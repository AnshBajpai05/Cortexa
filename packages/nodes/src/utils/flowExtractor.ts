...
export async function extractFlow(
  text: string,
  ctx: RunContext
): Promise<GraphSpec | null> {
  console.log(`[flowExtractor] Extracting mermaid flow from text (${text.length} chars)`);

  try {
    const result = await callNimStructured<GraphSpec>(
      EXTRACT_PROMPT + text.slice(0, 8000), // Limit context size
      {
        task: "agent",
        preference: "quality",
        temperature: 0.1,
        maxTokens: 2048,
        model: "meta/llama-3.1-8b-instruct", // Fast extraction!
      },
      ctx,
      (raw) => {
        try {
          ...
          if (typeof parsed.mermaid_code === "string" && parsed.mermaid_code.includes("graph")) {
            return { success: true, data: parsed as GraphSpec };
          }
