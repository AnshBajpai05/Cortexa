...
// ─── Config Schema ────────────────────────────────────────────────────────────
const GlobalEvaluatorNodeConfig = z.object({
  /** Heavy reasoning model for global pipeline evaluation */
  model: z
  ...
  globalRubric: z
    .string()
    .default(
      "Evaluate if the pipeline successfully completed its overarching goal and produced high-quality output."
    ),
});
...
// ─── Node Manifest ────────────────────────────────────────────────────────────
export const globalEvaluatorNode: NodeManifest = {
  id: "logic.global-evaluator",
  name: "Global Evaluator",
  description:
    "System-level evaluator powered by Kimi-K2 that judges the entire pipeline result for strategy and success.",
  ...
  async run(ctx: RunContext, rawConfig: unknown): Promise<NodeOutput> {
    ...
    const originalGoal = ctx.inputs["original_goal"] as string;

    if (!finalOutput || !originalGoal) {
      throw new Error(
        `[globalEvaluator] Missing required inputs: final_output or original_goal`
      );
    }
...
`;
    ...
    const evaluationString = await tools.llm("nvidia", userMessage, {
      model: config.model,
      systemPrompt,
      maxTokens: 500,
      temperature: 0.1,
