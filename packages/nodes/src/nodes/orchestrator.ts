...
// ─── Config Schema ────────────────────────────────────────────────────────────
const OrchestratorNodeConfig = z.object({
  /** GLM-4.7 is purpose-built for agentic routing decisions */
  model: z
...
// ─── Node Manifest ────────────────────────────────────────────────────────────
export const orchestratorNode: NodeManifest = {
  id: "logic.orchestrator",
  name: "Orchestrator",
  description:
    "Light planning node powered by GLM-4.7 that evaluates a goal against available options and decides the next route.",
  ...
  async run(ctx: RunContext, rawConfig: unknown): Promise<NodeOutput> {
    ...
    if (!goal || !optionsRaw) {
      throw new Error(
        `[orchestratorNode] Missing required inputs: goal or options (runId=${ctx.runId})`
      );
    }
...
`;
    ...
    const routingString = await tools.llm("nvidia", userMessage, {
      model: config.model,
      systemPrompt,
      maxTokens: 300,
      temperature: 0.1,
